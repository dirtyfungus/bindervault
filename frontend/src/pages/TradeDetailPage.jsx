...
import { ArrowLeft, Store, Package, CheckCircle } from 'lucide-react'
...
export default function TradeDetailPage() {
  ...
  const respondMutation = useMutation({
    mutationFn: (action) => api.post(`/trades/${offerId}/respond`, { action }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['offer', offerId] }); qc.invalidateQueries({ queryKey: ['trades'] }) },
      ...
      <div style={page}>
        ...
        <div style={grid}>
          ...
          <div style={card}>
            ...
            {offer.status === 'pending' && isSender && (
              <button className="btn btn-danger" style={{ marginTop: 20 }} onClick={() => respondMutation.mutate('cancel')}>Cancel offer</button>
            )}
          ...
          {/* Schedule panel (only when accepted) */}
